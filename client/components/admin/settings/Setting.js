...
export function Setting({ settingId, sectionChanged }) {
	...
	const hint = useMemo(() => t.has(i18nDescription) && <MarkdownText>{t(i18nDescription)}</MarkdownText>, [i18nDescription, t]);
	const callout = useMemo(() => alert && <RawText>{t(alert)}</RawText>, [alert, t]);
